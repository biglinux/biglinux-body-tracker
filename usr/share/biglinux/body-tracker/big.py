...
def tkTooltipChange(text, color, bg, mouseX, mouseY):
    ...
    # Remove the old Label widget before creating a new one
    for widget in tkTooltip.winfo_children():
        widget.pack_forget()
...
def make_action(action):
    ...
    if action == 'showOptions1':
        mouse.press(Button.left)
        tkTooltipChange('', "#000000", "#00ff00", mouse.position[0], mouse.position[1])
...
async def main():
    ...
    if args.rightEye:
        ...
        await asyncio.create_task(verify_false_click('rightEye', 0.7, 1, 'pressRight', 'releaseRight'))
    ...
    if args.enableKiss:
        # var_name, distance_value, confirm_value, action_start, action_end
        await asyncio.create_task(verify_false_click('kiss', 0.7, 1, 'scrollV', 'wait'))
...
with mp_face_mesh.FaceMesh(
    ...
    for idx, (frame, frame_rgb) in enumerate(source):
            ...
            if fpsBrightness > fpsRealMean:
                ...
                if results.multi_face_landmarks:
                    ...
                    if args.autoBrightness and eyes_roi_value is not None:
                        ...
                        if brightness_average < 100:
                            gain = 1
                            source.gain(gain)
                        elif brightness_average > 150:
                            gain = 0
                            source.gain(gain)
                        fpsBrightness = 0
        ...
        if results.multi_face_landmarks:
            ...
            ############################################
            # Criar landmarks
            # Nesta parte do código, são criados os pontos de referência (landmarks)
            # Os pontos de referência são usados para calcular o movimento do mouse, sua velocidade e aceleração.

            # Create landmarks
            # In this part of the code, the reference points (landmarks)
            ...
            if results.multi_face_landmarks is not None:
                ...
                if frameNumber < fpsRealMean:
                    ...
                    if args.mouseDetectionMode == 2:
                        ...
                        # Calcula a movimentação horizontal a partir do eixo x e z do ponto 6 (nariz)
                        # Calculate horizontal movement from point 6 (nose) x and z axis
                        # mouseMoveX = math.atan((landmarks[1][0] - landmarks[454][0]) + (landmarks[1][0] - landmarks[234][0])) * 180 
 

                        # Calcula a movimentação vertical a partir do eixo y e z do ponto 6 (nariz)
                        # Calculate vertical movement from point 6 (nose) y and z axis
                        # mouseMoveY = math.atan((landmarks[1][1] - landmarks[152][1]) + (landmarks[1][1] - landmarks[10][1])) * 180

                        mouseMoveX = - ((landmarks[6][0] + landmarks[352][0] + 2) - (landmarks[6][0] + landmarks[123][0] + 2)  * args.mouseSpeedX * 5)

                        # Calcula a movimentação vertical a partir do eixo y e z do ponto 6 (nariz)
                        # Calculate vertical movement from point 6 (nose) y and z axis
                        # mouseMoveY = ((math.atan((landmarks[1][1] - ((landmarks[152][1] + landmarks[473][1]) / 2)) + (landmarks[1][1] - ((landmarks[234][1] + landmarks[468][1]) / 2))) * 300))
                    ...
                    if args.mouseDetectionMode == 3:
                        ...
                        # Calcula a movimentação horizontal a partir do eixo x e z do ponto 6 (nariz)
                        # Calculate horizontal movement from point 6 (nose) x and z axis
                        mouseMoveX = ((math.atan((landmarks[1][0] + ((landmarks[454][0] + landmarks[473][0] + landmarks[152][0]) / 3)) + (landmarks[1][0] + ((landmarks[234][0] + landmarks[468][0] + landmarks[10][0]) / 3))) * 180 / math.pi * 2.2) * 100) - 12000

                        # Calcula a movimentação vertical a partir do eixo y e z do ponto 6 (nariz)
                        # Calculate vertical movement from point 6 (nose) y and z axis
                        mouseMoveY = ((math.atan((landmarks[1][1] - ((landmarks[152][1] + landmarks[473][1] + landmarks[34][1]) / 3)) + (landmarks[1][1] + ((landmarks[10][1] + landmarks[468][1] + landmarks[264][1]) / 3))) * 180 / math.pi * 2.2) * 100) - 10000
                    ...
                    ############################################
                    # Adaptação da primeira versão que utiliza a rotação da cabeça e outra forma de aceleração
                    # Adaptation of the first version that uses head rotation and another form of acceleration
                    ############################################
                    if args.mouseDetectionMode == 4:
                        ...
                        # Calcula a movimentação horizontal a partir do eixo x e z do ponto 6 (nariz)
                        # Calculate horizontal movement from point 6 (nose) x and z axis
                        mouseMoveX = (math.atan((landmarks[1][0] + ((landmarks[454][0] + landmarks[473][0] + landmarks[152][0]) / 3))) * 2)
                        mouseMoveX = mouseMoveX * mouseMoveX * mouseMoveX * args.mouseSpeedX
                        # irisLeftX = (math.atan((landmarks[468][0] + ((landmarks[454][0] + landmarks[473][0] + landmarks[152][0]) / 3))) * 20)
                        # mouseMoveX = -(noseX * ((irisLeftX - noseX)) * 10)

                        # irisLeftX = ((math.atan((landmarks[468][0] + ((landmarks[454][0] + landmarks[473][0] + landmarks[152][0]) / 3)) + (landmarks[468][0] + ((landmarks[234][0] + landmarks[468][0] + landmarks[10][0]) / 3))) * 180 / math.pi * 2.2) * 4)
                        # Calcula a movimentação vertical a partir do eixo y e z do ponto 6 (nariz)
                        # Caqlculate vertical movement from point 6 (nose) y and z axis
                        # mouseMoveY = ((math.atan((landmarks[1][1] - ((landmarks[152][1] + landmarks[473][1] + landmarks[34][1]) / 3)) + (landmarks[1][1] + ((landmarks[10][1] + landmarks[468][1] + landmarks[264][1]) / 3))) * 180 / math.pi * 2.2) * 4)

                        mouseMoveY = (math.atan((landmarks[1][1] + ((landmarks[152][1] + landmarks[473][1] + landmarks[34][1]) / 3))) * 2)
                        mouseMoveY = mouseMoveY * mouseMoveY * mouseMoveY * args.mouseSpeedY
                        # irisLeftY = (math.atan((landmarks[468][1] + ((landmarks[152][1] + landmarks[473][1] + landmarks[34][1]) / 3))) * 20)
                        # mouseMoveY = -(noseY * ((irisLeftY - noseY)) * 10)
                    ...
                    if (leftEye < leftEyeMean * 0.68 and rightEye < rightEyeMean * 0.68) or (leftEye < leftEyeNormalized * 0.4 and rightEye < rightEyeNormalized * 0.4) and not clicked:
                        eyesOpen = 0
                    ...
                    if eyesOpen > 0 and leftEye > leftEyeNormalized * 0.8 and rightEye > rightEyeNormalized * 0.8 and not clicked:
                        if leftEye > leftEyeOld * 0.85 and rightEye > rightEyeOld * 0.85  and mousePointXabs <= 1 and mousePointYabs <= 1 and mousePointXabsOld <= 1 and mousePointYabsOld <= 1:
                            eyesOpen = 10
                        else: eyesOpen = 5
                    ...
                    #         # print(mousePointYApply)
                        ...
                        #     mouthCenterLeft = np.linalg.norm(landmarks[214] - landmarks[87]) * 1000
                        ...
                        #     mouthCenterRight = np.linalg.norm(landmarks[434] - landmarks[317]) * 1000
